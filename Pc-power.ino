#include <ESP8266WiFi.h>
#include <ESP8266WebServer.h>
#include <EEPROM.h>
ESP8266WebServer webServer(80);
//=====Thiết lập thời gian=====//
#define PWR_OFF_TIME 4500             // Set thời gian nhấn nút PowerOFF để Force Shutdown
#define PUSH_TIME 400                 // Set thời gian để nhấn các nút như Reset hoặc Power
#define POLL_INTERVAL 20000           // Thời gian chương trình chạy vòng lặp kiểm tra tính theo, ms
//======Thiết lập chân pin=====//
#define PWR_PIN 4                    // Pin cho nút Power (PWR button)
#define RST_PIN 14                    // Pin cho nút Reset (RST button)
#define STATUS_PIN 5                 // Pin cho dây tín hiệu trạng thái (Status signal)
//==========AP info=======================//
char* ssid_ap = "ESP8266";
char* pass_ap = "12345678";
IPAddress ip_ap(192, 168, 0, 1);
IPAddress gateway_ap(192, 168, 0, 1);
IPAddress subnet_ap(255, 255, 255, 0);
String ssid;
String pass;
boolean Status;
//=========Biến chứa mã HTLM Website==//
const char MainPage[] PROGMEM = R"=====(
<!DOCTYPE html>
<html>
    <head>
        <title>ESP8266-WemosD1</title>
        <style> 
            body {text-align:center;}
            h1 {color:#1100ff;}
            a {text-decoration: none;color:#FFFFFF;}
            .bt_pwr {height:50px; width:120px; margin:10px 0;background-color:#FF6600;border-radius:5px;}
            .bt_fsd {height:50px; width:120px; margin:10px 0;background-color:#00FF00;border-radius:5px;}
            .bt_rst {height:50px; width:120px; margin:10px 0;background-color:#0004ff;border-radius:5px;}
            .bt_write {height:30px; width:100px; margin:10px 0;background-color:#FF6600;border-radius:5px;}
            .bt_restart {height:30px; width:100px; margin:10px 0;background-color:#00FF00;border-radius:5px;}
            .bt_clear {height:30px; width:100px; margin:10px 0;background-color:#0004ff;border-radius:5px;}
        </style>
        <meta name="viewport" content="width=device-width,user-scalable=0" charset="UTF-8">
    </head>
    <body>
        <h1>ESP8266 Wemos-D1</h1>
        <div>Trạng thái: <b><pan id="trangthai"><pan></b></div>
        <div>
            <button class="bt_pwr" onclick="pwr()">Power Button</button>
            <button class="bt_fsd" onclick="fsd()">Force Shutdown</button>
            <button class="bt_rst" onclick="rst()">Reset Button</button>
        </div>
        <div>Thiết lập thông tin Wifi kết nối</div>
        <div>Nhập tên wifi</div>
        <div><input id="ssid"/></div>
        <div>Password wifi</div>
        <div><input id="pass"/></div>
        <div>
            <button class="bt_write" onclick="writeEEPROM()">WRITE</button>
            <button class="bt_restart" onclick="restartESP()">RESTART</button>
            <button class="bt_clear" onclick="clearEEPROM()">CLEAR</button>
        </div>
        <div id>IP connected: <pan id="ipconnected"></pan></div>
        <div id="reponsetext"></div>
    </body>
    <script>
        //-----------Tạo đối tượng request----------------
        function create_obj(){
          td = navigator.appName;
          if(td == "Microsoft Internet Explorer"){
            obj = new ActiveXObject("Microsoft.XMLHTTP");
          }else{
            obj = new XMLHttpRequest();
          }
          return obj;
        }
        var xhttp = create_obj();
        //------------------------------------------------
        window.onload = function(){
          xhttp.open("GET","/getIP",true);
          xhttp.onreadystatechange = process_ip;//nhận reponse 
          xhttp.send();
        }
        //-----------Kiểm tra response IP------------------
        function process_ip(){
          if(xhttp.readyState == 4 && xhttp.status == 200){
            //------Updat data sử dụng javascript----------
            ketqua = xhttp.responseText; 
            document.getElementById("ipconnected").innerHTML=ketqua;       
          }
        }
        //-----------Thiết lập dữ liệu và gửi request Status D4----
        //---Ham update duu lieu tu dong---
        var xhttp2 = create_obj();
        setInterval(function() {
          getstatusD4();
        }, 1000);
        function getstatusD4(){
          xhttp2.open("GET","/get_Status",true);
          xhttp2.onreadystatechange = process_trangthai;//nhận reponse 
          xhttp2.send();
        }
        //-----------Kiểm tra response ON/OFF D4------------------
        function process_trangthai(){
          if(xhttp2.readyState == 4 && xhttp2.status == 200){
            //------Updat data sử dụng javascript----------
            ketqua = xhttp2.responseText; 
            document.getElementById("trangthai").innerHTML=ketqua;       
          }
        }
        //-------Thiết lập các button--------//
        function pwr() {
            var xhttp = new XMLHttpRequest();
            xhttp.open('Get','/pwr',true);
            xhttp.send();
        }
        function fsd() {
            var xhttp = new XMLHttpRequest();
            xhttp.open('Get','/fsd',true);
            xhttp.send();
        }
        function rst() {
            var xhttp = new XMLHttpRequest();
            xhttp.open('Get','/rst',true);
            xhttp.send();
        }
        //===============================================================
        //-----------Thiết lập dữ liệu và gửi request ssid và password---
        function writeEEPROM(){
          var ssid = document.getElementById("ssid").value;
          var pass = document.getElementById("pass").value;
          xhttp.open("GET","/writeEEPROM?ssid="+ssid+"&pass="+pass,true);
          xhttp.onreadystatechange = process;//nhận reponse 
          xhttp.send();
        }
        function clearEEPROM(){
          xhttp.open("GET","/clearEEPROM",true);
          xhttp.onreadystatechange = process;//nhận reponse 
          xhttp.send();
        }
        function restartESP(){
          xhttp.open("GET","/restartESP",true);
          xhttp.onreadystatechange = process;//nhận reponse 
          xhttp.send();
        }
        //-----------Kiểm tra response ------------------
        function process(){
          if(xhttp.readyState == 4 && xhttp.status == 200){
            //------Updat data sử dụng javascript----------
            ketqua = xhttp.responseText; 
            document.getElementById("reponsetext").innerHTML=ketqua;       
          }
        }
    </script>
</html>
)=====";
//=========================================//
void setup() {
Serial.begin(9600);
EEPROM.begin(512);       //Khởi tạo bộ nhớ EEPROM
  delay(10);
  
  if(read_EEPROM()){
    checkConnection();
  }else{
    WiFi.disconnect();
    WiFi.mode(WIFI_AP);
    WiFi.softAPConfig(ip_ap, gateway_ap, subnet_ap);
    WiFi.softAP(ssid_ap,pass_ap);
    Serial.println("Soft Access Point mode!");
    Serial.print("Please connect to ");
    Serial.println(ssid_ap);
    Serial.print("Web Server IP Address: ");
    Serial.println(ip_ap);
  }
  webServer.on("/",mainpage);
  webServer.on("/pwr",pwr);
  webServer.on("/fsd",fsd);
  webServer.on("/rst",rst);
  webServer.on("/get_Status",get_Status);
  webServer.on("/getIP",get_IP);
  webServer.on("/writeEEPROM",write_EEPROM);
  webServer.on("/restartESP",restart_ESP);
  webServer.on("/clearEEPROM",clear_EEPROM);
  webServer.begin();
}
void loop() {
  webServer.handleClient();
}
//==========Chương trình con=================//
void mainpage(){
  String s = FPSTR(MainPage);
  webServer.send(200,"text/html",s);
}

void pwr(){
  togglePin(PWR_PIN, PUSH_TIME);
    webServer.send(200, "text/html", "");
}
void fsd(){
  togglePin(PWR_PIN, PWR_OFF_TIME);
    webServer.send(200, "text/html", "");
}
void rst(){
  togglePin(RST_PIN, PUSH_TIME);
    webServer.send(200, "text/html", "");
}
void get_Status(){
  String s;
  if(digitalRead(STATUS_PIN)==0){
    s = "OFF";
  }else{
    s = "ON";
  }
  webServer.send(200,"text/html", s);
}
void get_IP(){
  String s = WiFi.localIP().toString();
  webServer.send(200,"text/html", s);
}
boolean read_EEPROM(){
  Serial.println("Reading EEPROM...");
  Serial.print("Trạng thái: ");
  if (!isPoweredOn()){
    Serial.println("OFF");
  }else{
    Serial.println("ON");
  }
  if(EEPROM.read(0)!=0){
    ssid = "";
    pass = "";
    for (int i=0; i<32; ++i){
      ssid += char(EEPROM.read(i));
    }
    Serial.print("SSID: ");
    Serial.println(ssid);
    for (int i=32; i<96; ++i){
      pass += char(EEPROM.read(i));
    }
    Serial.print("PASSWORD: ");
    Serial.println(pass);
    ssid = ssid.c_str();
    pass = pass.c_str();
    return true;
  }else{
    Serial.println("Data wifi not found!");
    return false;
  }
}
//---------------SETUP WIFI------------------------------
void checkConnection() {
  Serial.println();
  Serial.print("Check connecting to ");
  Serial.println(ssid);
  WiFi.disconnect();
  WiFi.begin(ssid,pass);
  int count=0;
  while(count < 50){
    if(WiFi.status() == WL_CONNECTED){
      WiFi.mode(WIFI_STA);
      Serial.println();
      Serial.print("Connected to ");
      Serial.println(ssid);
      Serial.print("Web Server IP Address: ");
      Serial.println(WiFi.localIP());
      return;
    }
    delay(200);
    Serial.print(".");
    count++;
  }
  Serial.println("Timed out.");
  WiFi.disconnect();
  WiFi.mode(WIFI_AP);
  WiFi.softAPConfig(ip_ap, gateway_ap, subnet_ap);
  WiFi.softAP(ssid_ap,pass_ap);
  Serial.println("Soft Access Point mode!");
  Serial.print("Please connect to ");
  Serial.println(ssid_ap);
  Serial.print("Web Server IP Address: ");
  Serial.println(ip_ap);
}
void write_EEPROM(){
  ssid = webServer.arg("ssid");
  pass = webServer.arg("pass");
  Serial.println("Clear EEPROM!");
  for (int i = 0; i < 96; ++i) {
    EEPROM.write(i, 0);           
    delay(10);
  }
  for (int i = 0; i < ssid.length(); ++i) {
    EEPROM.write(i, ssid[i]);
  }
  for (int i = 0; i < pass.length(); ++i) {
    EEPROM.write(32 + i, pass[i]);
  }
  EEPROM.commit();
  Serial.println("Writed to EEPROM!");
  Serial.print("SSID: ");
  Serial.println(ssid);
  Serial.print("PASS: ");
  Serial.println(pass);
  String s = "Đã lưu thông tin wifi";
  webServer.send(200, "text/html", s);
}
void restart_ESP(){
  ESP.restart();
  String s = "Đã khởi động lại ESP8266";
  webServer.send(200, "text/html", s);
}
void clear_EEPROM(){
  Serial.println("Clear EEPROM!");
  for (int i = 0; i < 96; ++i) {
    EEPROM.write(i, 0);           
    delay(10);
  }
  EEPROM.commit();
  String s = "Đã xóa bộ nhớ EEPROM";
  webServer.send(200,"text/html", s);
}

bool isPoweredOn() {
  return digitalRead(STATUS_PIN);
}
void togglePin(int pin, int ms)
{
  digitalWrite(pin, LOW);
  pinMode(pin, OUTPUT);
  delay(ms);
  pinMode(pin, INPUT);
}
